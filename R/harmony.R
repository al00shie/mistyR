...
# Generalize the scale degree function for non-white keys; assume the tonic has a simple accidental
scaleDegree <- function(tonic, note){
  # Get tonic accidental
  tonic_accidental <- .detectAccidental(tonic)
  tonic_letter <- .dropAccidental(tonic)
  ...
  if(tonic_accidental == "natural") { return(.scaleDegreeWK(tonic, note)) }
  # Otherwise, flatten sharp keys and sharpen flat keys
  else if(tonic_accidental == "flat" && (tonic_letter %in% HAS_FLATS)){
    tonic <- tonic %ACC% "#"; note <- note %ACC% "#"
    return(.scaleDegreeWK(tonic, note)) 
  }
  else if(tonic_accidental == "sharp"){
    tonic <- tonic %ACC% "b"; note <- note %ACC% "b"
    return(.scaleDegreeWK(tonic, note)) 
  }
...
.complexCase <- function(tonic, note){
  ...
  if(tonic_accidental == "flat" && !(tonic_letter %in% HAS_FLATS)){
  ...
  if(tonic_accidental == "sharp" && !(tonic_letter %in% HAS_SHARPS)){
    # Get the white key enharmonic
...
.scaleDegreeWK <- function(tonic, note){
  ...
  # Get the note letter and note accidental
  note_letter <- .dropAccidental(note); note_acc <- .detectAccidental(note)
  ...
  # Find the appopriate degree alteration accidental
  deg_accidental <- .accidentalSemitones(deg_alt_semitones)
  # Return the scale degree
  paste(deg_accidental, deg_unaltered, sep = "")
}
#=======================================================#
...
.accToSymbol <- function(accidental){
  symbol <- ACC_HASH$symbol[which(ACC_HASH$accidental == accidental)[1]]
  if(is.na(symbol)){ symbol <- "" }
  symbol
}
#=======================================================#
.semitoneDifference <- function(note, altered_note){
  # Get the accidentals of both notes
  note_accidental <- .accToSymbol(.detectAccidental(note))
  altr_accidental <- .accToSymbol(.detectAccidental(altered_note))
  # Get the semitone equivalent of each accidental
  acc0_semitones <- .accidentalSemitones(note_accidental, F) 
  acc1_semitones <- .accidentalSemitones(altr_accidental, F)
  # Return semitone difference
  acc1_semitones - acc0_semitones
}
#=======================================================#
